        ...
        if ($resultado_saldo->num_rows>0) {
            ...
            if ($saldo_actual >= $monto) {
                ...
                if ($conn->affected_rows>0) {
                    ...
                    $sql = "UPDATE swift SET SALDO = SALDO + $monto WHERE CUENTA = $cuenta_destino";
                    $result = $conn->query($sql);

                    if ($conn->affected_rows>0) {
                        header("location: alertaExito.html");
                    } else {
                        header("location: alertaActC.html");
                    }
                } else {
                    header("location: alertaTrans.html");
                }
            } else {
                header("location: alertaFondosIt.html");
            }
        } else {
            header("location: alertaObSalt.html");
        }
